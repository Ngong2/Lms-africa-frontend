  ...
  const handleSubmit = async () => {
    ...
    try {
      if (method === 'Flutterwave') {
        const res = await axios.post('https://backend-ilw9.onrender.com/api/flutterwave/payment', { name, email, amount });
        console.log(res);
        window.location.href = res.data.paymentLink;

      } else if (method === 'MPesa') {
        const res = await axios.post('https://backend-ilw9.onrender.com/api/mpesa/payment', { phone, amount });
        console.log(res.data.CustomerMessage);
        setMessage(res.data.CustomerMessage || "Something went wrong please try again");

      } else if (method === 'Stripe') {
        const res = await axios.post('https://backend-ilw9.onrender.com/api/stripe/payment', { name, email, amount });
